...
class lane_controller(object):
    ...
    def setGains(self):
        ...
        self.k_d = self.k_d_value #self.setupParameter("~k_d",k_d_fallback)     # P gain for d
        
        self.k_theta =  self.k_theta_value #self.setupParameter("~k_theta",k_theta_fallback)     # P gain for theta
    ...
    def getGains_event(self, event):
        ...
        if params_old != params_new:
            ...
            self.v_bar = v_bar
            self.k_d = self.k_d_value #k_d
            self.k_theta =  self.k_theta_value #k_theta
            self.d_thres = d_thres
            self.d_ref = d_ref
            self.phi_ref = phi_ref
            self.theta_thres = theta_thres
            self.d_offset = d_offset + self.d_offset_value
            self.k_Id = k_Id
            self.k_Iphi = k_Iphi
    ...
    def PoseHandling(self, input_pose_msg, pose_source):
        ...
        rospy.loginfo('k_d: ' + str(self.k_d) + ' .. k_theta: ' + str(self.k_theta))
        rospy.loginfo('d_offset: ' + str(self.d_offset))
    ...
    def cbPose(self, pose_msg):
        ...
        car_control_msg.v = car_control_msg.v * self.velocity_to_m_per_s * self.gain_factor
        car_control_msg.omega = omega * self.omega_to_rad_per_s * self.gain_factor
